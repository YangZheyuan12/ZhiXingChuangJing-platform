...
-- =====================================================
-- 使用说明：
-- 1. 确保已执行 01_asset_library_enhancement.sql
-- 2. 执行此脚本填充 15 件真实文物数据
-- 3. 执行时间：约 10 秒
-- =====================================================
...
-- 检查是否已有数据，避免重复插入
SET @count = (SELECT COUNT(*) FROM museum_resources WHERE title LIKE '%蛋壳黑陶高柄杯%' OR title LIKE '%后母戊鼎%' OR title LIKE '%清明上河图%');
...
IF @count = 0 THEN
    ...
    SELECT '✅ 示例数据填充完成！共添加 15 件真实文物数据。' AS result;
ELSE
    SELECT 'ℹ️ 数据库中已存在示例数据，跳过填充。' AS result;
END IF;
